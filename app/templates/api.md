...
Sparv's API is available at [`[URL]`]([URL]).
...
[`[URL]/?text=En+exempelmening+till+nättjänsten`]([URL]/?text=En+exempelmening+till+nättjänsten)

POST requests are also supported using the same address. This can be useful for longer texts.
Here is an example using curl:

```.bash
curl -X POST --data-binary text="En exempelmening till nättjänsten" [URL]/
```
...
```.bash
curl -X POST -F files[]=@/path/to/file/myfile.txt [URL]/upload?
```
...
This object must satisfy the JSON schema that is generated by the backend
when provided with a `language` and a `mode` parameter:

[`[URL]/schema?language=sv&mode=plain`]([URL]/schema?language=sv&mode=plain)
...
[`[URL]/?text=Det+trodde+jag+aldrig.&settings={"positional_attributes":{"dependency_attributes":["ref","dephead","deprel"],"lexical_attributes":[],"compound_attributes":[]}}`]([URL]/?text=Det+trodde+jag+aldrig.&settings={"positional_attributes":{"dependency_attributes":["ref","dephead","deprel"],"lexical_attributes":[],"compound_attributes":[]}})

Or as a POST request using curl:

```.bash
curl -X POST -g --data-binary text="Det trodde jag aldrig." '[URL]/?settings={"positional_attributes":{"dependency_attributes":["ref","dephead","deprel"],"lexical_attributes":[],"compound_attributes":[]}}'
```
...
[`[URL]/makefile?settings={"positional_attributes":{"dependency_attributes":["ref","dephead","deprel"],"lexical_attributes":[],"compound_attributes":[]}}`]([URL]/makefile?settings={"positional_attributes":{"dependency_attributes":["ref","dephead","deprel"],"lexical_attributes":[],"compound_attributes":[]}})
...
[`[URL]/join?hash=57fce7e430c7ab4dd83d5244b566dade92595db2`]([URL]/join?hash=57fce7e430c7ab4dd83d5244b566dade92595db2)
...
[`[URL]/?text=Nun+folgt+ein+deutscher+Beispielsatz.&language=de`]([URL]/?text=Nun+folgt+ein+deutscher+Beispielsatz.&language=de)
...
[`[URL]/schema?language=de`]([URL]/schema?language=de)
...
`[URL]/?text=Nu+med+inkrementell+information&incremental=true`
...
* **parameters:**
    * `text` (required)
    * `settings`, default: settings returned from `/schema`
    * `language`, default: `sv`
    * `mode`, default: `plain`, options: `plain`, `xml`, `file`
    * `incremental`, default: `False`
* **examples:**
    * [`/?text=En+exempelmening+till+nättjänsten`]([URL]/?text=En+exempelmening+till+nättjänsten)
    * `curl -X POST --data-binary text="En exempelmening till nättjänsten" [URL]/`
* **result:**
...
* **example:** [`/schema?language=sv&mode=plain`]([URL]/schema?language=sv&mode=plain)
...
* **parameters:**
    ...
    * `settings`
    * `incremental`, default: `False`
* **examples:**  [`/makefile?settings={"positional_attributes":{"dependency_attributes":["ref","dephead","deprel"],"lexical_attributes":[],"compound_attributes":[]}}`]([URL]/makefile?settings={"positional_attributes":{"dependency_attributes":["ref","dephead","deprel"],"lexical_attributes":[],"compound_attributes":[]}})
* **result:**
...
* **parameters:**
    ...
    * `settings`
    * `incremental`, default: `False`
* **example:** `curl -X POST -F files[]=@/path/to/file/myfile.txt [URL]/upload?`
...
* **parameters:**
    * `hash` (required)
* **example:** `[URL]/download?hash=a0c3861b251a595c83859c6cf4c595e8c71ad8da-f`
...
* **parameters:**
    * `hash` (required)
    * `incremental`, default: `False`
* **examples:** [`/join?hash=57fce7e430c7ab4dd83d5244b566dade92595db2`]([URL]/join?hash=57fce7e430c7ab4dd83d5244b566dade92595db2)
* **result:**
